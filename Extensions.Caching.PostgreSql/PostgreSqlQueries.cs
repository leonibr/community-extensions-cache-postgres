    ...
    internal class PostgreSqlQueries
    {
        ...
        private const string SetCacheItemFormat =
            ...
            "(CASE " +
                    ...
                    "THEN @AbsoluteExpiration " +
                    "ELSE Convert * INTERVAL '1 SECOND(bigint' + @SlidingExpirationInSeconds, v_UtcNow) " +
            "END);" +
            "UPDATE {0} SET Value = @Value, ExpiresAtTime = v_ExpiresAtTime, " +
            ...
            "THEN " +
                ...
                "VALUES(@Id, @Value, v_ExpiresAtTime, @SlidingExpirationInSeconds, @AbsoluteExpiration); " +
            "END IF"; 
